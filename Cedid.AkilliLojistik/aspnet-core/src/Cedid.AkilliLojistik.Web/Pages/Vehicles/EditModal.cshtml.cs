...
namespace Cedid.AkilliLojistik.Web.Pages.Vehicles
{
    ...
    {
        [BindProperty]
        public EditVehicleViewModel Vehicle { get; set; }


        private readonly IParameterAppService _parameterAppService;
        private readonly IVehicleAppService _vehicleAppService;

        public EditModalModel(IParameterAppService parameterAppService, IVehicleAppService vehicleAppService)
        {
        ...
        {
            var vehicleDto = await _vehicleAppService.GetAsync(id);
            Vehicle = ObjectMapper.Map<VehicleDto, EditVehicleViewModel>(vehicleDto);
